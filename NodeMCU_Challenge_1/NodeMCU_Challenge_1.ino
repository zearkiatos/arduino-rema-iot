...
#define HOSTNAME "p.caprilespe"

// Conexión a Mosquitto
const char MQTT_HOST[] = "iotlab.virtual.uniandes.edu.co";
const int MQTT_PORT = 8082;
// Usuario uniandes sin @uniandes.edu.co
const char MQTT_USER[] = "p.caprilespe";
...
const char MQTT_PUB_TOPIC1[] = "humedad/santiago-de-chile/" HOSTNAME;
// Tópico al que se enviarán los datos de temperatura
const char MQTT_PUB_TOPIC2[] = "temperatura/santiago-de-chile/" HOSTNAME;

const char MQTT_BRIGHTNESS_TOPIC[] = "luminosidad/santiago-de-chile/" HOSTNAME;
...
void loop() {
  ...
  char brightnessMessage[json.length() + 1];
  json.toCharArray(brightnessMessage, json.length() + 1);
  ...
  if (!isnan(h) && !isnan(t) && !isnan(ldrValue)) {
    ...
    client.publish(MQTT_BRIGHTNESS_TOPIC, brightnessMessage, false);
  }
  ...
  Serial.print(" -> ");
  Serial.println(brightnessMessage);
